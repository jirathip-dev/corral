...
- `e1431422d5147eb89f0d592178a8cccd52b54ce4` — the production SOURCE commit (the
  per-live-session transport). Its source is unchanged at every later head:
  `git diff e143142..ee89f95 -- ios/FleetNotifier` is empty.
- `ee89f95a1355611c4df4d1af9349010cec76a0fb` — the head every gate and probe in
  this report was run at (it adds the #554 tests and evidence over `e143142`).
- The `.report.md`/`docs/evidence` text you are reading is the only later delta;
  `git diff --check` and `check-release-demo.py` (the two gates whose scope
  covers docs/pins) were re-run afterwards.
...
| G4 Release build | `xcodebuild build -configuration Release -sdk iphonesimulator … CODE_SIGNING_ALLOWED=NO` | `RELEASE_EXIT=65` in the chain (**ENOSPC**: `lipo: can't write to output file … .dSYM/…/FleetNotifier.lipo (No space left on device)`, host at 116 MiB free), then `g554-release_EXIT=0` on the standalone re-run with the same command; log `/tmp/g554-release.log` |
