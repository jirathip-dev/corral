...
4. *(Optional)* **Notifications** for state-change alerts (start / blocked / done). Normal setup asks for no notification permission and needs no APNs credentials; background APNs delivery awaits the host-side provisioning checkpoint (an APNs `.p8` auth key + `CORRAL_APNS_*` env, see [docs/PUSH.md](docs/PUSH.md)). Simulator and DEBUG builds use the local notification bridge.
