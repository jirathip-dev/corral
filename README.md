...
4. *(Optional)* **Notifications** for state-change alerts (start / blocked / done). The board and recents work without granting notification permission and need no APNs credentials — and nothing asks for them on its own: the app never prompts and never registers for push at install, pairing, or first live board. Turning **State-change notifications** on in Settings is the only trigger (then the OS permission prompt appears, and only a grant enables). Onboarding completes without ever visiting it. Background APNs delivery awaits the host-side provisioning checkpoint (an APNs `.p8` auth key + `CORRAL_APNS_*` env, see [docs/PUSH.md](docs/PUSH.md)); simulator and DEBUG builds use the local notification bridge.
