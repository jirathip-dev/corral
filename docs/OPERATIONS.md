...
| Daemon log storms: repeated `events.subscribe` `REQUEST_TIMEOUT` + re-bootstrap, fd count climbing | herdr replays pane state BEFORE answering `subscribe`; a replay flood that outruns the client's event sink used to backpressure the reader and starve the response. The reader is now non-blocking (events are best-effort and dropped when the channel is full; the response is critical-path) and a dropped client aborts the reader so no descriptor is leaked (#105) |
