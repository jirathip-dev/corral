...
// MARK: - #210 fleet-health strip (HEALTH ONLY — no spend)

@MainActor
final class FleetHealthStripTests: XCTestCase {
    private func healthEntry(
        name: String,
        orchAlive: Bool = true,
        orchState: String? = "working",
        workers: Int = 2,
        heartbeat: UInt64? = 1_000_000,
        degraded: Bool = false,
        paused: Bool = false,
        warnings: [String] = []
    ) -> FleetHealthEntry {
    ...
    func testSnapshotDecodesFleetHealthAndToleratesOlderDaemonWithoutIt() throws {
        let withHealth = Data(#"""
        {"schema_version":5,"rev":7,"generated_at":100,"agents":{},
         "fleet_health":[{"name":"corral","gh_repo":"jirathip-dev/corral","paused":false,
          "orch":"orch-corral","orch_alive":true,"orch_state":"working","workers":2,
          "last_heartbeat":996000,"degraded":false,"warnings":[]}]}
        """#.utf8)
        let decoded = try JSONDecoder().decode(Snapshot.self, from: withHealth)
        XCTAssertEqual(decoded.fleetHealth.count, 1)
        XCTAssertEqual(decoded.fleetHealth[0].name, "corral")
        XCTAssertTrue(decoded.fleetHealth[0].orchAlive)
        XCTAssertEqual(decoded.fleetHealth[0].workers, 2)
        XCTAssertEqual(decoded.fleetHealth[0].lastHeartbeat, 996_000)

        // An older daemon (no fleet_health key) must still decode: the strip
        // simply reads empty, never fails the stream.
        let legacy = Data(#"{"schema_version":5,"rev":8,"generated_at":200,"agents":{}}"#.utf8)
        let legacyDecoded = try JSONDecoder().decode(Snapshot.self, from: legacy)
        XCTAssertTrue(legacyDecoded.fleetHealth.isEmpty,
                      "missing fleet_health key must default to an empty strip")

        // The daemon omits an EMPTY `warnings` array (serde
        // skip_serializing_if): the row must decode to `[]`, not fail the
        // whole snapshot.
        let noWarnings = Data(#"""
        {"schema_version":5,"rev":9,"generated_at":300,"agents":{},
         "fleet_health":[{"name":"corral","gh_repo":"jirathip-dev/corral","paused":false,
          "orch":"orch-corral","orch_alive":true,"orch_state":"working","workers":1,
          "last_heartbeat":950000,"degraded":false}]}
        """#.utf8)
        let tolerant = try JSONDecoder().decode(Snapshot.self, from: noWarnings)
        XCTAssertEqual(tolerant.fleetHealth[0].warnings, [],
                       "absent warnings key must decode to an empty array")
    }

    func testProjectionMarksHealthyDegradedAndPausedFleets() {
        let now: UInt64 = 1_004_000
        let lines = BoardModel.fleetHealthLines(
            [
                healthEntry(name: "corral", workers: 2, heartbeat: 1_000_000),
                healthEntry(name: "sendmeter", orchAlive: false, orchState: nil, workers: 0, heartbeat: nil,
                      degraded: true, warnings: ["orch_missing"]),
                healthEntry(name: "plush", orchAlive: false, orchState: nil, workers: 0, heartbeat: nil,
                      paused: true),
            ],
            now: now
        )
        XCTAssertEqual(lines.count, 3)
        XCTAssertEqual(lines[0].marker, "●")
        XCTAssertTrue(lines[0].orchAlive)
        XCTAssertEqual(lines[0].workerCount, 2)
        XCTAssertEqual(lines[0].heartbeatAgeLabel, "♥4s")
        XCTAssertFalse(lines[0].degraded)

        XCTAssertEqual(lines[1].marker, "⚠")
        XCTAssertFalse(lines[1].orchAlive)
        XCTAssertEqual(lines[1].heartbeatAgeLabel, "♥—")
        XCTAssertTrue(lines[1].degraded)

        XCTAssertEqual(lines[2].marker, "⏸")
        XCTAssertTrue(lines[2].paused)
        XCTAssertFalse(lines[2].degraded, "paused fleets never read as degraded")
    }

    func testHeartbeatAgeFormatting() {
        let now: UInt64 = 10_000_000_000
        XCTAssertEqual(BoardModel.heartbeatAge(at: now - 4_000, now: now), "4s")
        XCTAssertEqual(BoardModel.heartbeatAge(at: now - 180_000, now: now), "3m")
        XCTAssertEqual(BoardModel.heartbeatAge(at: now - 7_500_000, now: now), "2h 05m")
        XCTAssertEqual(BoardModel.heartbeatAge(at: now - 26 * 3_600_000, now: now), "1d 2h")
    }

    func testFleetStoreSnapshotAppliesHealthRows() {
        let store = FleetStore()
        let snapshot = Snapshot(
            ...
            generatedAt: 100,
            agents: [:],
            fleetHealth: [healthEntry(name: "corral")]
        )
        store.apply(.snapshot(snapshot))
