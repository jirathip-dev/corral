...
//! round-trip per poll, ~531ms measured) and emits [`GhRepoState`] facts into
//! the [`PlaneSink`]: open PRs (state, mergeability, head oid, CI status
...
//!   disconnect); the plane holds no receiver itself, so a silent daemon
//!   reads 0. In the never-connected state the plane wakes every 2s on an
//!   in-process timer ONLY to re-check the count — no network traffic.
//!
...
//! warning is logged and the plane stays down — no crash loop, no retry.
//!
...
//! successful poll for that repo; the first successful poll emits every repo.
//! Failed polls emit nothing and leave the last-known state intact.
//!
...
/// In-process wake interval used ONLY to re-check the subscriber count while
/// never-connected; no network traffic happens in that state.
...
enum CadenceAction {
    ...
    RecheckSubscribers,
    /// Active mode, not yet due: sleep until the given poll deadline.
    SleepUntil(Instant),
    /// Due now (or first subscriber just connected): poll, then schedule the
    /// next poll after `next` (foreground or background cadence).
    Poll { next: Duration },
}
...
fn cadence_step(
    ...
    if !ever_connected {
        if subscribers > 0 {
            // First subscriber ever: immediate SWR fetch, then foreground.
...
impl GhPlane {
    ...
    async fn run_forever(self: Arc<Self>, sink: PlaneSink) {
        ...
        loop {
            ...
            let (action, ever_connected_now) =
                cadence_step(ever_connected, subscribers, next_poll, Instant::now(), &self.config);
            ...
            match action {
                ...
                CadenceAction::Poll { next } => {
                    match self.poll_once(&token, &query, &mut last, &sink).await {
                        ...
                        Err(e) => warn!(error = %e, "gh plane poll failed"),
                    }
    ...
    /// that repo and keep its last-known state.
    async fn poll_once(
        &self,
        token: &str,
        query: &str,
        ...
        for (i, repo) in TRACKED_REPOS.iter().enumerate() {
            ...
            if last.get(repo.name).is_some_and(|prev| *prev == state) {
                continue;
            }
            changed += 1;
            last.insert(repo.name.to_string(), state.clone());
            if sink.send(PlaneEvent::Gh(state)).await.is_err() {
                return Err(GhError::SinkClosed);
            }
        }
        info!(latency_ms, repos = TRACKED_REPOS.len(), changed, "gh plane round-trip complete");
        Ok(())
    }
}
...
/// The per-item contexts are collapsed (any failing item fails the PR; else
/// any in-flight item -> PENDING; else all green -> SUCCESS); when the
/// contexts list is empty or absent, the rollup's aggregate `state` is
/// mapped instead. An empty/absent rollup -> UNKNOWN.
fn collapse_ci(rollup: Option<&RollupWire>) -> String {
    if let Some(rollup) = rollup {
        ...
        return match rollup.state.as_deref() {
...
fn collapse_items(items: &[RollupItemWire]) -> String {
...
mod tests {
    ...
    fn collapse_ci_priorities_and_unknowns() {
        ...
        // ACTION_REQUIRED contributes nothing -> UNKNOWN.
    ...
    fn maps_repo_wire_into_contract_types() {
        ...
        assert_eq!(state.prs[0].mergeable, "CONFLICTING");
        assert_eq!(state.prs[0].ci_status, "SUCCESS");
        assert_eq!(state.prs[0].head_sha, "abc123");
        assert_eq!(state.prs[1].ci_status, "PENDING", "empty contexts -> aggregate state");
        assert_eq!(state.prs[1].head_sha, "", "null headRefOid -> empty string");
    ...
    fn cadence_swr_zero_polling_without_any_subscriber() {
        ...
        for tick in 0..500 {
            ...
            let (action, ever) = cadence_step(ever_connected, 0, next_poll, now, &config);
            ever_connected = ever;
            next_poll = None;
    ...
    fn cadence_first_subscriber_triggers_immediate_fetch() {
        let config = GhPlaneConfig::default();
        let (action, ever) = cadence_step(false, 1, None, fake_now(), &config);
    ...
    fn cadence_foreground_60s_while_connected() {
        ...
        let (action, _) = cadence_step(true, 1, Some(t0 - Duration::from_secs(1)), t0, &config);
        ...
        let (action, _) = cadence_step(true, 1, Some(deadline), t0, &config);
        assert_eq!(action, CadenceAction::SleepUntil(deadline));
        // Due again exactly 60s later.
        let (action, _) = cadence_step(true, 1, Some(t0), t0 + FOREGROUND_POLL, &config);
        assert_eq!(action, CadenceAction::Poll { next: FOREGROUND_POLL });
    }
    ...
    fn cadence_background_300s_after_all_subscribers_disconnect() {
        let t0 = fake_now();
        let config = GhPlaneConfig::default();
        // Ever connected, nobody live, due -> poll, 300s cadence.
        let (action, _) = cadence_step(true, 0, Some(t0 - Duration::from_secs(1)), t0, &config);
        assert_eq!(action, CadenceAction::Poll { next: BACKGROUND_POLL });
        // Just polled with nobody watching -> next poll scheduled 300s out.
        let (action, _) = cadence_step(true, 0, Some(t0 + Duration::from_secs(10)), t0, &config);
        assert_eq!(action, CadenceAction::SleepUntil(t0 + Duration::from_secs(10)));
        let (action, _) = cadence_step(true, 0, Some(t0), t0 + BACKGROUND_POLL, &config);
        assert_eq!(action, CadenceAction::Poll { next: BACKGROUND_POLL });
    }

    #[test]
    fn cadence_reconnects_to_foreground_when_subscriber_returns() {
        let t0 = fake_now();
        let config = GhPlaneConfig::default();
        // Was in background, a subscriber comes back -> foreground again.
        let (action, _) = cadence_step(true, 1, Some(t0 - Duration::from_secs(1)), t0, &config);
        assert_eq!(action, CadenceAction::Poll { next: FOREGROUND_POLL });
    }
}
