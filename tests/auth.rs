...
async fn http_app() -> (Arc<AuthPlane>, tempfile::TempDir, axum::Router) {
    ...
    let app = router(AppState { store, auth: auth.clone() });
    (auth, dir, app)
}
...
async fn drive_seam_full_flow_ac1_and_ac3_over_http() {
    ...
    let v = read_json(res).await;
    assert_eq!(v["error_type"], "not_granted");
    ...
    assert_eq!(res.status(), StatusCode::UNAUTHORIZED);
    assert_eq!(read_json(res).await["error_type"], "bad_signature");
...
async fn drive_seam_requires_step_up_for_destructive_payloads() {
    ...
    let res = app.clone().oneshot(post("/drive", serde_json::to_value(&sd).unwrap())).await.unwrap();
    assert_eq!(res.status(), StatusCode::FORBIDDEN);
    assert_eq!(read_json(res).await["error_type"], "step_up_required");
...
async fn live_daemon_self_test() {
    ...
    assert_eq!(res.json::<serde_json::Value>().await.unwrap()["error_type"], "not_granted");

    // 4. Promote (admin) -> signed drive executes (AC1).
    ...
    assert_eq!(res.json::<serde_json::Value>().await.unwrap()["ok"], true);
